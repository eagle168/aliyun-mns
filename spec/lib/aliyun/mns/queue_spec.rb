...
describe Aliyun::Mns::Queue do
  ...
  describe ".queues" do
    ...
    specify "find all queues" do
      expect(Aliyun::Mns::Request).to receive(:get).with("/", mns_headers:{}).and_return xml_response
    ...
    specify "query queues" do
      expect(Aliyun::Mns::Request).to receive(:get).with("/", mns_headers:{"x-mns-prefix"=>"query"}).and_return xml_response
      queues = Aliyun::Mns::Queue.queues(query: "query")
    end

    specify "find number of queues" do
      expect(Aliyun::Mns::Request).to receive(:get).with("/", mns_headers:{"x-mns-ret-number"=>5}).and_return xml_response
      queues = Aliyun::Mns::Queue.queues(size: 5)
    end

    specify "find of queues start at given position" do
      expect(Aliyun::Mns::Request).to receive(:get).with("/", mns_headers:{"x-mns-marker"=>2}).and_return xml_response
      queues = Aliyun::Mns::Queue.queues(offset: 2)
    end
  end
  ...
  describe "#delete" do
    specify "will delete existing queue" do
      expect(Aliyun::Mns::Request).to receive(:delete).with("/aQueue")
  ...
  describe "#receive_message" do
    ...
    specify "will receive message from a queue" do
      expect(Aliyun::Mns::Request).to receive(:get).with("/aQueue/messages",{}).and_return xml_response
    ...
    specify "will receive message from a queue with poll wait" do
      expect(Aliyun::Mns::Request).to receive(:get).with("/aQueue/messages",params:{waitseconds: 60}).and_return xml_response
  ...
  describe "#peek" do
    ...
    specify "will peek message of a queue" do
      expect(Aliyun::Mns::Request).to receive(:get).with("/aQueue/messages",params:{peekonly: true}).and_return xml_response

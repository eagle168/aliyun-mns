...
describe Aliyun::Mns::Queue do
  ...
  let(:xml_message){
    Aliyun::Mns::Message.new(Aliyun::Mns::Queue["aQueue"], <<-XML)
  ...
  describe "#delete" do
    specify "will delete the message from queue" do
      expect(Aliyun::Mns::Request).to receive(:delete).with("/aQueue/messages", params:{:ReceiptHandle=>"MbZj6wDWli+QEauMZc8ZRv37sIW2iJKq3M9Mx/KSbkJ0"})
      xml_message.delete
    end
  ...
  describe "#change_visibility" do
    specify "will change message's visibility timeout" do
      expect(Aliyun::Mns::Request).to receive(:put).with("/aQueue/messages", params:{
